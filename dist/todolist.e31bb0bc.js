...
var todoInput = document.querySelector('.todo-input'); //grab todo item

var dateInput = document.querySelector('.date-input'); //grab date input

var todoButton = document.querySelector('.todo-button'); //generates add todo event

var todoList = document.querySelector('.todo-list'); //used for delete/check events
...
var sortList = new Array(); //stores todo names and dates to be sorted in array of structs

var completedArr = new Array(); //stores completed todos to be displayed at bottom of list

function addTodo(event) {
  event.preventDefault(); //prevent browser refresh

  if (todoInput.value == "" || todoInput.value == " ") {
    //error handling for blank todo inputs
  ...
  if (!dateInput.value) {
    //error handling for blank date input
  ...
  sortList.sort(function (a, b) {
    ...
    var dateA = a.date;
    var dateB = b.date;
    return (0, _dateFns.compareAsc)(dateA, dateB);
  });
  ...
  for (var i = 0; i < sortList.length; i++) {
    //goes through sorted list in order and displays the todos
    ...
    var dateHold = document.createElement("input");
  ...
  for (var i = 0; i < completedArr.length; i++) {
    var _todoDiv = document.createElement('div');

    _todoDiv.classList.add("todo"); //overarching div that holds the todo object


    _todoDiv.classList.toggle("completed");

    var _newTodo = document.createElement('li');

    _newTodo.innerHTML = completedArr[i].name + '<span class="tab"></span>';

    _newTodo.classList.add('todo-item');

    _todoDiv.appendChild(_newTodo); //adds todo task name into todo object


    var _dateHold = document.createElement("input");

    _dateHold.setAttribute("type", "hidden");

    _dateHold.setAttribute("class", "date-hold");
    ...
    _todoDiv.appendChild(_dueDate); //adds due date into todo object


    var _completedButton = document.createElement('button');

    _completedButton.innerText = 'Check';

    _completedButton.classList.add("complete-btn");
